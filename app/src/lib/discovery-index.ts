import { getAllPrinciples, getCanonicalProductionContent, getGoldStandardAssessment, getGoldStandardExercises, getGoldStandardSessions } from './content-bridge';
...
export function getDiscoveryIndex():DiscoveryItem[]{
  const problems=getProblems().map(p=>({id:p.problem_id,type:'PROBLEMĂ' as const,title:p.human_title,summary:p.observable_behavior,href:`/rezolva-pe-teren/${p.slug}`,searchText:norm([p.human_title,p.slug,p.observable_behavior,p.what_is_seen,p.problem_id==='PRB-0003'?'scanare scanează privire peste umăr':'',...p.possible_explanations.map(x=>x.text),...p.child_cues.map(x=>x.say)].join(' ')),gameMoment:p.family,evidence:p.research_state,media:p.flagship}));
  const principles=getCanonicalProductionContent(getAllPrinciples()).map(p=>({id:p.id,type:'PRINCIPIU' as const,title:p.title,summary:p.summary,href:`/principii/${p.slug}`,searchText:norm(`${p.title} ${p.summary} ${p.target_behavior} ${p.child_wording}`),gameMoment:'PEDAGOGIE_TACTICĂ',evidence:'EVIDENCE_LINKED',media:Boolean(p.tactical_visual_hooks)}));
...
export function explainDiscovery(item:DiscoveryItem,query:string){const q=norm(query);if(q&&norm(item.title).includes(q))return 'Potrivire în titlu';if(q&&norm(item.summary).includes(q))return 'Potrivire în obiectiv sau observație';return q?'Potrivire în conținutul canonic':'Conținut canonic disponibil';}
